...
import { getActors, deleteActorById } from 'apiSdk/actors';
import { ActorInterface } from 'interfaces/actor';

type ColumnType = ColumnDef<ActorInterface, unknown>;
...
export function ActorListPage(props: ActorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getActors({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'nationality.contains', 'gender.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ActorInterface>>(
    () => `/actors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ActorInterface) => {
    if (hasAccess('actor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/actors/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'nationality', header: 'Nationality', accessorKey: 'nationality' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/actors/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('actor', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/actors/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('actor', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="actor" />
        </Flex>

        {hasAccess('actor', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/actors/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
