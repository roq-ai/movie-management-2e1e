...
import { getDirectors, deleteDirectorById } from 'apiSdk/directors';
import { DirectorInterface } from 'interfaces/director';

type ColumnType = ColumnDef<DirectorInterface, unknown>;
...
export function DirectorListPage(props: DirectorListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DirectorInterface>>(
    () => `/directors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DirectorInterface) => {
    if (hasAccess('director', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/directors/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/directors/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('director', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/directors/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('director', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="director" />
        </Flex>

        {hasAccess('director', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/directors/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
