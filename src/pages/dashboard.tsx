...
import { getActors } from 'apiSdk/actors';
import { ActorInterface } from 'interfaces/actor';
import { getDirectors } from 'apiSdk/directors';
...
function DashboardPage() {
  ...
  const actorStat = useSWR<PaginatedInterface<ActorInterface>>(
    () => `/actors`,
    () => getActors({ limit: 1, offset: 0 }),
  );

  const directorStat = useSWR<PaginatedInterface<DirectorInterface>>(
    () => `/directors`,
    () => getDirectors({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Actors',
      result: actorStat?.data?.totalCount,
      isLoading: actorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Directors',
      result: directorStat?.data?.totalCount,
      isLoading: directorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reviews',
