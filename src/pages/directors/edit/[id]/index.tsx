...
import { getDirectorById, updateDirectorById } from 'apiSdk/directors';
...
function DirectorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DirectorInterface>(
    () => (id ? `/directors/${id}` : null),
    () => getDirectorById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DirectorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDirectorById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<DirectorInterface>({
    initialValues: data,
